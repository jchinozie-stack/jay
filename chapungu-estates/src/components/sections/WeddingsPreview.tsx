            ...
            <ul className="space-y-4 mb-10" role="list">
              {features.map(({ icon: Icon, text }) => (
                <li key={text} className="flex items-center gap-3">
                  <div className="w-8 h-8 bg-brand-50 flex items-center justify-center shrink-0">
                    <Icon className="w-4 h-4 text-brand-500" aria-hidden="true" />
                  </div>
                  <span className="font-body text-earth-700">{text}</span>
                </li>
              ))}
            </ul>

            <div className="flex gap-4">
              <Link href="/weddings" className="btn-gold">Explore Packages</Link>
              <Link href="/contact?type=wedding" className="btn-outline-gold">Send Enquiry</Link>
            </div>
          </div>
        </div>
      </div>
